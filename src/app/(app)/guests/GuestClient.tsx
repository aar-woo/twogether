...
import { Guest, GUEST_SIDES, GUEST_RELATIONSHIPS } from "../../../../types/index";
...
export function GuestClient({ weddingId: _weddingId, guests }: GuestClientProps) {
  ...
  const [optimisticGuests, setOptimisticGuests] = useOptimistic(guests, guestReducer);
  ...
  function handleAddGuest() {
    ...
    startTransition(async () => {
      ...
      const result = await createGuest(newName.trim(), newSide, newRelationship, newInvited);
      if (result?.error) {
  ...
  function handleToggleInvited(id: string, currentInvited: boolean) {
    ...
    if (!guest) return;
    startTransition(async () => {
      setOptimisticGuests({ type: "update", guest: { ...guest, invited: !currentInvited } });
  ...
  const sideCounts = GUEST_SIDES.map((s) => ({
    label: s,
    count: optimisticGuests.filter((g) => g.side === s).length,
  })).filter((s) => s.count > 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent className="pb-4 px-4">
            <span className="text-2xl font-semibold text-foreground">{totalGuests}</span>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="pb-4 px-4">
            <span className="text-2xl font-semibold text-foreground">{invitedCount}</span>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex flex-wrap gap-2 items-end">
              ...
              <Select value={newSide} onValueChange={(val) => setNewSide(val as string)}>
      ...
      {/* Table */}
      {(optimisticGuests.length > 0 || showAddForm) && optimisticGuests.length > 0 && (
          ...
          <div className="divide-y divide-sage-100">
            {optimisticGuests.map((g) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Select
                      ...
                      onValueChange={(val) => setEditRelationship(val as string)}
                    >
                      <SelectTrigger className="h-8 w-full">
                        <SelectValue />
                      </SelectTrigger>
              ...
              return (
                <div
                  key={g.id}
                  className="grid grid-cols-[1fr_100px_120px_80px_80px] items-center gap-2 px-2 py-3 hover:bg-sage-50"
                >
                  <span className="text-sage-900 font-medium truncate">{g.name}</span>
                  <span className="text-sage-600 text-sm">{g.side ?? "—"}</span>
                  <span className="text-sage-600 text-sm">{g.relationship ?? "—"}</span>
                  <button
