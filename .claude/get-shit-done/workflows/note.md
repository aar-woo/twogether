...
- **Global scope**: `/Users/aaronwoo/claude/twogether/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/aaronwoo/claude/twogether/.claude/notes/`)
...
2. Glob `/Users/aaronwoo/claude/twogether/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/aaronwoo/claude/twogether/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/aaronwoo/claude/twogether/.claude/notes/` — works in any directory
